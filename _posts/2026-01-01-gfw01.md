...
[中文版本](https://medium.com/@xnvlnasna/gfw-深度解析-01-架构-f7cbf9e48fb4){:target="_blank"}

<figure style="text-align:center;">
    <img src="/assets/img/posts/gfw01/cover.png" width="70%">
</figure>

In the late 1990s and early 2000s, China's Internet infrastructure expanded rapidly, and during this period the National Computer Network and Information Security Management Center (CNCERT) spearheaded the development of the Great Firewall (GFW). The physical layout of the early Chinese Internet provided a natural entry point: most international traffic passed through a handful of centralized exchange points. These "choke points" were natural places to plug in filtering without redesigning the entire domestic network.

The earliest versions of the GFW were built around these gateways. At each one, border routers enforced access-control rules, external appliances tapped and inspected traffic, and forged packets interrupted offending connections. The infrastructure was run by state-affiliated telecom operators working alongside newly established regulators tasked with information control.
...
In principle, users can bypass poisoned resolvers by querying foreign DNS servers. The GFW counters by injecting forged responses when it detects outbound DNS queries crossing the border. These forged responses almost always arrive first, since they are generated locally rather than traveling to Japan, the US, or elsewhere. By the DNS protocol, the client accepts the first answer and ignores the genuine one when it finally shows up.
