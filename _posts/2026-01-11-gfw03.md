...
[中文版本](https://medium.com/@xnvlnasna/gfw-深度解析-03-深度包检测-5a19bdee58a5){:target="_blank"}

<figure style="text-align:center;">
    <img src="/assets/img/posts/gfw03/cover.png" width="70%">
</figure>

The GFW is far more than a traditional firewall. Architecturally, it resembles a large-scale Intrusion Detection and Prevention System (IDPS) of the kind deployed in enterprise networks, government agencies, and financial institutions: it observes traffic, analyzes behavior, identifies policy violations, and blocks the offending traffic.
...
To manage the enormous traffic volume on national backbone links, the GFW deploys a distributed architecture backed by datacenter-scale compute.
...
Unlike a conventional TCP/IP stack, which must handle full bidirectional communication, the GFW only needs to parse inbound traffic from one direction. That simplification lets the system fan out parallel TCP-stack instances across cores.
...
Empirical observations suggest that the GFW keys this temporary blacklist using a 3-tuple: (client IP, server IP, server port). This choice reflects a practical compromise. By applying a coarse-grained but short-lived blacklist, the GFW can significantly increase blocking effectiveness while limiting the collateral damage caused by potential misclassification.
